...
function Home() {
  ...
  const categories = ["All", "Footwear", "Clothing", "Electronics",
    "Accessories", "Furniture", "Laptops", "Mobiles"];
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-6">
          <div className="flex flex-wrap gap-3 items-center justify-between">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl border transition font-medium text-sm
                  ${showFilters ? 'bg-purple-600 text-white border-purple-600' : 'bg-white text-gray-600 border-gray-300 hover:border-purple-400'}`}>
                🔧 Filters
                {activeFilterCount > 0 && (
            ...
            <p className="text-gray-500 text-sm">
              <span className="font-bold text-gray-800">{filtered.length}</span> products found
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {activeCategory !== "All" && (
              <span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-sm flex items-center gap-1">
                📂 {activeCategory}
                <button onClick={() => setActiveCategory("All")} className="ml-1 hover:text-purple-900">✕</button>
              </span>
            )}
            {priceRange.min && (
              <span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-sm flex items-center gap-1">
                Min ₹{priceRange.min}
                <button onClick={() => setPriceRange({ ...priceRange, min: "" })} className="ml-1">✕</button>
              </span>
            )}
            {priceRange.max && (
              <span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-sm flex items-center gap-1">
                Max ₹{priceRange.max}
                <button onClick={() => setPriceRange({ ...priceRange, max: "" })} className="ml-1">✕</button>
              </span>
            )}
            {inStockOnly && (
              <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm flex items-center gap-1">
                ✅ In Stock Only
                <button onClick={() => setInStockOnly(false)} className="ml-1">✕</button>
              </span>
            )}
            {sortBy !== "newest" && (
              <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm flex items-center gap-1">
                🔃 {sortBy === "price_low" ? "Price: Low→High" : sortBy === "price_high" ? "Price: High→Low" : "Top Rated"}
    ...
    <div className="bg-white rounded-2xl shadow hover:shadow-xl transition-all duration-300 overflow-hidden group">
      ...
      <div className="p-4">
        <div className="flex justify-between items-center mt-1">
          ...
          <p className={`text-xs font-medium ${product.stock > 0 ? "text-green-500" : "text-red-500"}`}>
        ...
        {quantity === 0 ? (
          ...
          <div className="mt-4 flex items-center justify-between bg-purple-50 rounded-xl px-3 py-2">
            ...
            <span className="font-bold text-purple-700 text-base">{quantity} added</span>
            <button
