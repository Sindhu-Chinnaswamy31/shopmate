...
function ProductForm({ form, onSubmit, onCancel, onChange, title, errors = {} }) {
  return (
        ...
        <form onSubmit={onSubmit} className="flex flex-col gap-3">
          {["name", "description", "price", "category", "stock", "image"].map((field) => (
            <div key={field}>
              ...
              />
              {errors[field] && (
                <p className="text-red-500 text-xs mt-1">⚠️ {errors[field]}</p>
              )}
            </div>
          ))}
...
function AdminPanel() {
  ...
  const validateProduct = (form) => {
    ...
    if (form.stock === "" || form.stock < 0) errors.stock = "Valid stock is required";
    return errors;
  };
  ...
  const handleAddProduct = async (e) => {
    ...
    setProductErrors({});
    try {
      await adminAddProduct({ ...form, price: Number(form.price), stock: Number(form.stock) });
  ...
  const handleEditProduct = async (e) => {
    ...
    setProductErrors({});
    try {
      await adminUpdateProduct(editProduct._id, { ...form, price: Number(form.price), stock: Number(form.stock) });
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
        ...
        {activeTab === "dashboard" && dashboard && (
          <div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              {[
                { label: "Total Products", value: dashboard.totalProducts, icon: "📦", color: "bg-blue-500", bg: "bg-blue-50" },
                { label: "Total Orders", value: dashboard.totalOrders, icon: "🧾", color: "bg-green-500", bg: "bg-green-50" },
                { label: "Total Users", value: dashboard.totalUsers, icon: "👥", color: "bg-yellow-500", bg: "bg-yellow-50" },
                { label: "Revenue", value: `₹${dashboard.totalRevenue.toLocaleString()}`, icon: "💰", color: "bg-purple-500", bg: "bg-purple-50" },
              ].map(stat => (
                <div key={stat.label} className={`${stat.bg} rounded-2xl p-6 border border-gray-100`}>
                  <div className={`${stat.color} w-12 h-12 rounded-xl flex items-center justify-center text-2xl mb-4`}>
                    {stat.icon}
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              ...
              <div className="bg-white rounded-2xl shadow p-6">
                <h3 className="font-bold text-gray-800 mb-4">📈 Revenue (Last 6 Months)</h3>
                <ResponsiveContainer width="100%" height={250}>
                  <LineChart data={dashboard.revenueByMonth}>
                    ...
                    <Line type="monotone" dataKey="revenue" stroke="#7C3AED"
                      strokeWidth={3} dot={{ fill: "#7C3AED", strokeWidth: 2, r: 5 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow p-6">
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <PieChart>
                    <Pie
                      data={[
                        { name: "Pending", value: dashboard.ordersByStatus.pending, fill: "#FCD34D" },
                        { name: "Paid", value: dashboard.ordersByStatus.paid, fill: "#34D399" },
                        { name: "Shipped", value: dashboard.ordersByStatus.shipped, fill: "#60A5FA" },
                        { name: "Delivered", value: dashboard.ordersByStatus.delivered, fill: "#7C3AED" },
                        { name: "Failed", value: dashboard.ordersByStatus.failed, fill: "#F87171" },
                      ].filter(d => d.value > 0)}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {/* New Users */}
              <div className="bg-white rounded-2xl shadow p-6">
                <h3 className="font-bold text-gray-800 mb-4">👥 New Users (Last 6 Months)</h3>
                <ResponsiveContainer width="100%" height={250}>
        ...
        {activeTab === "products" && (
          <div>
            <div className="flex justify-between items-center mb-6">
              ...
              <button
                onClick={() => { setShowAddProduct(true); setForm({ name: "", description: "", price: "", category: "", stock: "", image: "" }); }}
                ...
                <tbody className="divide-y divide-gray-100">
                  {products.map((p) => (
                    <tr key={p._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <span className="bg-purple-100 text-purple-600 px-2 py-1 rounded-full text-xs">{p.category}</span>
                      </td>
                      <td className="px-4 py-3 font-semibold text-purple-600">₹{p.price}</td>
                      <td className="px-4 py-3">
                        <span className={`font-semibold ${p.stock > 0 ? "text-green-500" : "text-red-500"}`}>{p.stock}</span>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          <button
                            onClick={() => { setEditProduct(p); setForm({ name: p.name, description: p.description, price: p.price, category: p.category, stock: p.stock, image: p.image || "" }); }}
                ...
                <tbody className="divide-y divide-gray-100">
                  {orders.map((o) => (
                    <tr key={o._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 font-semibold text-purple-600">₹{o.totalAmount}</td>
                      <td className="px-4 py-3">
      ...
      {/* Modals */}
      {showAddProduct && (
        <ProductForm form={form} onSubmit={handleAddProduct} onChange={handleFormChange}
          onCancel={() => { setShowAddProduct(false); setProductErrors({}); }}
          title="Add New Product" errors={productErrors} />
      )}
      {editProduct && (
        <ProductForm form={form} onSubmit={handleEditProduct} onChange={handleFormChange}
          onCancel={() => { setEditProduct(null); setProductErrors({}); }}
          title="Edit Product" errors={productErrors} />
      )}
    </div>
  );
}
